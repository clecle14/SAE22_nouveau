...
// Gestion d'erreur plus robuste pour l'API
...
async function setupCityAutocomplete() {
    ...
    const debouncedSearch = debounce(async (value) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/location/cities?token=${API_TOKEN}&search=${encodeURIComponent(value)}`);
            const data = await response.json();
            
            if (data.cities && data.cities.length > 0) {
                autocompleteContainer.innerHTML = data.cities.slice(0, 5).map(city => {
                    // Construire l'affichage selon les données disponibles
                    let displayText = city.name;
                    let additionalInfo = '';
                    
   
                    if (city.cp) {
                        additionalInfo = city.cp; // Code postal
                    } else if (city.insee) {
                        additionalInfo = `INSEE: ${city.insee}`; // Code INSEE
                    } else if (city.latitude && city.longitude) {
                        additionalInfo = `${city.latitude.toFixed(2)}°, ${city.longitude.toFixed(2)}°`; // Coordonnées
                    }
                    
                    return `
                        <div class="suggestion" data-city="${city.name}" data-insee="${city.insee || ''}" data-cp="${city.cp || ''}">
                            <span class="city-name">${displayText}</span>
                            ${additionalInfo ? `<span class="postal-code">${additionalInfo}</span>` : ''}
                        </div>
                    `;
                }).join('');
            } else {
                autocompleteContainer.innerHTML = '<div class="suggestion no-results">Aucune ville trouvée</div>';
            }
        } catch (error) {
            console.error('Erreur autocomplétion:', error);
            autocompleteContainer.innerHTML = '<div class="suggestion error">Erreur de recherche</div>';
        }
    }, 300);
    ...
    autocompleteContainer.addEventListener('click', (e) => {
        if (e.target.closest('.suggestion') && !e.target.closest('.suggestion').classList.contains('no-results') && !e.target.closest('.suggestion').classList.contains('error')) {
    ...
    cityInput.addEventListener('keydown', (e) => {
        const suggestions = autocompleteContainer.querySelectorAll('.suggestion:not(.no-results):not(.error)');
        ...
        if (e.key === 'ArrowDown') {
            ...
            cityInput.value = selectedSuggestion.dataset.city;
            autocompleteContainer.innerHTML = '';
            selectedIndex = -1;
